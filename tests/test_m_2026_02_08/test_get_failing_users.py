...
@pytest.fixture
def students_data_no_marks():
    """Фикстура с тестовыми данными пользователей и их оценок."""
    return {
        'Иванов': {'Математика': [], 'Физика': []},
        'Петров': {'Математика': [], 'Физика': [5, 4, 5]},
        'Попов': {'Математика': [4, 4, 3], 'Физика': [3, 5, 5]},
        'Кузнецов': {'Математика': [5, 5, 5], 'Физика': [3, 4, 4]}
        }
...
def test_get_failing_users_no_marks(students_data_no_marks):
    expected = {'Иванов', 'Петров'}
    result = get_failing_users(students_data_no_marks)
    assert expected == result
